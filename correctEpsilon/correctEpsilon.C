...
namespace fv
{
    defineTypeNameAndDebug(correctEpsilon, 0);
    addToRunTimeSelectionTable(option, correctEpsilon, dictionary);
}
}
...
Foam::fv::correctEpsilon::correctEpsilon
(
    const word& name,
...
{
    fieldNames_.setSize(1, epsilonName_);
...
bool Foam::fv::correctEpsilon::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        coeffs_.readEntry("epsilonName", epsilonName_);
        coeffs_.readEntry("kName", kName_);
...
void Foam::fv::correctEpsilon::correct(volScalarField& S)
{
    const volScalarField &alpha_ = mesh_.lookupObject<volScalarField>(alphaName_);
    const volScalarField &k_ = mesh_.lookupObject<volScalarField>(kName_);
    ...
    for (const label celli : cells_)
    {
        ...
        {
            S[celli] = B_*6.*nut_[celli]*k_[celli]/deltaX/deltaX;
            nCorrect++;
        }
    ...
    Info <<"multiphaseEpsilonCorrect: "<< nCorrect << " cells are corrected in total "<< nTotal << " cells." << endl;
