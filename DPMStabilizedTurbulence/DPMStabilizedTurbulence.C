...
#include "DPMStabilizedTurbulence.H"
...
namespace Foam
{
    namespace fv
    {
        defineTypeNameAndDebug(DPMStabilizedTurbulence, 0);
        addToRunTimeSelectionTable(option, DPMStabilizedTurbulence, dictionary);
    } // namespace fv
} // namespace Foam

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::fv::DPMStabilizedTurbulence::DPMStabilizedTurbulence(
    ...
    : option(sourceName, modelType, dict, mesh),
      ...
      alpha_(dimensionedScalar::getOrAddToDict("alpha", coeffs_, 1.36))
{
...
void Foam::fv::DPMStabilizedTurbulence::addSup(const volScalarField &rho,
                                               fvMatrix<scalar> &eqn,
                                               const label fieldi)
{
...
void Foam::fv::DPMStabilizedTurbulence::addSup(fvMatrix<scalar> &eqn,
                                               const label fieldi)
{
    if (fieldi != 0)
    {
    ...
    const auto *turbPtr = mesh_.findObject<incompressible::turbulenceModel>(turbulenceModel::propertiesName);
    ...
    const auto &rhoc = mesh_.lookupObject<volScalarField>(rhoName_);
    const auto &alphac = mesh_.lookupObject<volScalarField>(alphaName_);
    ...
    const volScalarField rhoMix
    (
        "rhoMix",
        rhoc * alphac + rhop * (1. - alphac)
    );

    const auto &g = meshObjects::gravity::New(mesh_.time());

    const dimensionedScalar eps0
    (
        "eps0",
        epsilon.dimensions(),
        SMALL
    ...
    (
        "GbyK",
        Cmu_*k/(epsilon+eps0)*alpha_*(g&fvc::grad(rhoMix))/rhoMix
    );

    return eqn -= fvm::SuSp(GbyK, k);
}

void Foam::fv::DPMStabilizedTurbulence::correct(volScalarField &field)
{
    ...
    const dimensionedScalar pSmall
    (
        "pSmall", 
        dimless / sqr(dimTime), 
        SMALL
    ...
    (
        max(epsilon, lambda2_ * C_ * pRat * epsilon)
    );
